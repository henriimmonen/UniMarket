...
CREATE TABLE listings (id SERIAL PRIMARY KEY , header TEXT, location TEXT, content TEXT, user_id INTEGER REFERENCES users(id), visible BOOLEAN);
...
CREATE TABLE messages (id SERIAL PRIMARY KEY, content TEXT, sent_at TIMESTAMP, sent_by INTEGER REFERENCES users(id));